...
export default function WellnessJourney() {
        ...
        const fetchDashboardData = async () => {
            try {
                ...
                const [
                    ...
                    fetch(`${apiConfig.baseUrl}/appointments`, { headers }),
                    fetch(`${apiConfig.baseUrl}/mood-history`, { headers }),
                    fetch(`${apiConfig.baseUrl}/dashboard/activity-summary`, { headers }),
                    fetch(`${apiConfig.baseUrl}/journals`, { headers }),
                ]);

                if (!appointmentsRes.ok || !moodHistoryRes.ok || !activitySummaryRes.ok || !journalsRes.ok) {
                    // Silent fail during polling or throw?
                    // For now, if it's polling, we just log and continue to avoid clearing UI.
                    if (isLoading) throw new Error("Failed to fetch dashboard data");
                    else {
                        console.warn("Polling failed to fetch update");
                        return;
                    }
                }
    ...
    useEffect(() => {
        // ... (keep logic same but ensure we verify time strictly)
        if (upcomingAppointments.length === 0) return;
        const checkTimes = () => {
            const now = new Date().getTime();
            const newJoinableIds = new Set<number>();
            upcomingAppointments.forEach(app => {
                // Modified: check strictly for active modes
                if (['video_call', 'voice_call'].includes(app.mode) && app.status === 'booked') {
                    const appointmentTime = new Date(app.appointment_time).getTime();
                    const timeUntilAppointment = appointmentTime - now;
                    // For testing/demo, allow joining 10 mins before and 50 mins after.
                    // Or if it's "now"
                    const tenMinutesInMillis = 10 * 60 * 1000;
                    if (timeUntilAppointment <= tenMinutesInMillis && timeUntilAppointment > -50 * 60 * 1000) {
    ...
    return (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* ... (left column with charts kept same) ... */}
                                ...
                                upcomingAppointments.map((appointment) => (
                                    <div key={appointment.id} className="bg-primary/10 border-l-4 border-primary/50 p-4 rounded-lg">
                                        ...
                                        {joinableSessions.has(appointment.id) && (
                                            <button
                                                onClick={() => handleJoinSession(appointment.meeting_link)}
                                                className="mt-4 inline-flex items-center justify-center w-full px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 transition"
                                            >
