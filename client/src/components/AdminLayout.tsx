...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, title, icon, username = "Admin", actions }) => {
    ...
    const NavItem = ({ href, icon: Icon, label }: { href: string; icon: any; label: string }) => {
        ...
        return (
            <Link href={href}>
                <a className={`flex items-center space-x-4 p-2 rounded-lg transition-colors ${isActive ? "bg-gray-800 text-blue-400 font-semibold" : "hover:bg-gray-800 text-gray-100"}`}>
                    <Icon size={22} />
                    <span>{label}</span>
                </a>
            </Link>
    ...
    return (
        <div className="flex h-screen bg-gray-100 relative">
            {/* Mobile Header */}
            <div className="md:hidden fixed top-0 left-0 right-0 bg-gray-900 text-white p-4 flex justify-between items-center z-50 shadow-md">
            ...
            {/* Sidebar */}
            <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-gray-100 p-6 flex flex-col justify-between transform transition-transform duration-300 ease-in-out md:static md:translate-x-0
      ...
      `}>
                ...
                <button
                    ...
                    className="flex items-center space-x-4 p-2 rounded-lg text-red-400 hover:bg-gray-800 transition-colors w-full mb-8 md:mb-0"
                >
                    <LogOut size={22} />
                    <span>Logout</span>
                </button>
