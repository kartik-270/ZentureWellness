...
import { ChevronDown, ChevronUp, Star, Wind, Flame, X } from "lucide-react";
...
function WellnessModal({ onClose, streak }: { onClose: () => void; streak: number }) {
...
export default function DailyWellnessCorner() {
    const [open, setOpen] = useState(false);
    ...
    return (
        <>
            ...
            <div
                ...
                className="bg-card rounded-xl shadow-sm border border-border overflow-hidden cursor-pointer group hover:border-purple-400/60 hover:shadow-lg transition-all duration-300 flex flex-col"
            >
                {/* Gradient thumbnail */}
            ...
            {/* Full activity modal */}
            {open && <WellnessModal onClose={() => setOpen(false)} streak={streak} />}
        </>
    );
}
